...
app.post('/api/admin/questions', authenticateToken, requireAdmin, (req, res) => {
    const { quiz_id, question_text, question_type, correct_answer, option_a, option_b, option_c, option_d, explanation } = req.body;
    
    if (!quiz_id || !question_text || !correct_answer) {
        return res.status(400).json({ message: 'Quiz ID, question text and correct answer are required' });
    }
    ...
    db.query(query, [quiz_id, question_text, question_type || 'multiple_choice', correct_answer, option_a, option_b, option_c, option_d, explanation], (err, result) => {
        if (err) {
            return res.status(500).json({ message: 'Database error', error: err });
        }
        ...
        db.query(updateQuery, [quiz_id, quiz_id], (updateErr) => {
            if (updateErr) {
...
app.put('/api/admin/questions/:id', authenticateToken, requireAdmin, (req, res) => {
    ...
    db.query(query, [question_text, question_type, correct_answer, option_a, option_b, option_c, option_d, explanation, questionId], (err) => {
        if (err) {
            return res.status(500).json({ message: 'Database error', error: err });
        }
